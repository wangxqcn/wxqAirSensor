...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	...
	// 数据点"湿度"对应的标识名
	protected static final String KEY_HUMI = "Humi";
	// 数据点"光照强度"对应的标识名
	protected static final String KEY_LIGHT = "Light";
	// 数据点"二氧化碳"对应的标识名
	protected static final String KEY_CO2 = "Co2";
	// 数据点"设备MAC地址"对应的标识名
	protected static final String KEY_DEVICEMAC = "DeviceMac";
	// 数据点"设备硬件版本"对应的标识名
	protected static final String KEY_DEVICEHARDVER = "DeviceHardVer";
	// 数据点"设备软件版本"对应的标识名
	protected static final String KEY_DEVICESOFTVER = "DeviceSoftVer";
	// 数据点"设备IP地址"对应的标识名
	protected static final String KEY_DEVICEIP = "DeviceIp";
	...
	// 数据点"湿度"对应的存储数据
	protected static double data_Humi;
	// 数据点"光照强度"对应的存储数据
	protected static int data_Light;
	// 数据点"二氧化碳"对应的存储数据
	protected static int data_Co2;
	// 数据点"设备MAC地址"对应的存储数据
	protected static byte[] data_DeviceMac;
	// 数据点"设备硬件版本"对应的存储数据
	protected static byte[] data_DeviceHardVer;
	// 数据点"设备软件版本"对应的存储数据
	protected static byte[] data_DeviceSoftVer;
	// 数据点"设备IP地址"对应的存储数据
	protected static byte[] data_DeviceIp;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				...
				if (dataKey.equals(KEY_HUMI)) {
			
					if (map.get(dataKey) instanceof Integer) {
						data_Humi = (Integer) map.get(dataKey);
					} else {
						data_Humi = (Double) map.get(dataKey);
					}
				}
				...
				if (dataKey.equals(KEY_DEVICEMAC)) {
			
					data_DeviceMac = (byte[]) map.get(dataKey);
				}
				if (dataKey.equals(KEY_DEVICEHARDVER)) {
			
					data_DeviceHardVer = (byte[]) map.get(dataKey);
				}
				if (dataKey.equals(KEY_DEVICESOFTVER)) {
			
					data_DeviceSoftVer = (byte[]) map.get(dataKey);
				}
				if (dataKey.equals(KEY_DEVICEIP)) {
			
					data_DeviceIp = (byte[]) map.get(dataKey);
				}
			}
		}
